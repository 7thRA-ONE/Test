...
async function prodia() {
  try {
   ...
   // console.log(modelsList)
    ...
    const modelNumber = 1; // Replace this with the actual model number input
...
async function generateImage(params) {
  const options = {
    ...
    headers: {
      ...
      'X-Prodia-Key': 'ae9d60f9-8a5c-4a9c-895d-b6f2bcf70d86'
    },
    data: params
  ...
  if (request.data.status === "queued") {
    const options = {
      method: 'GET',
      url: `https://api.prodia.com/v1/job/${request.data.job}`,
      headers: {
        accept: 'application/json',
        'X-Prodia-Key': 'ae9d60f9-8a5c-4a9c-895d-b6f2bcf70d86'
      }
    ...
    let job = await axios.request( options);

    while (job.data.status !== "succeeded" && job.data.status !== "failed") {
      await new Promise((resolve) => setTimeout(resolve, 2500));
      job = await axios.request(options);
    }
